...
const pub = require('../msg/pub');

console.log("Config was " + JSON.stringify(config));

exports.getDataSet = (id, cb) => {
  ds.get(ds.key(['DataSet', id]), cb);
}

exports.getBucket = (datasetid, cb) => {
...
exports.streamFile = (datasetid, id, cb) => {
  ds.get(ds.key(['DataSet', datasetid]), (err, dataset) => {
    if (err) {
      return cb(err);
    }
...
exports.getFiles = (bucketName, cb) => {
  var bucket = storage.bucket(bucketName);
  bucket.getFiles(cb);
}

exports.getDataSets = (repocode, cb) => {
  var query = ds.createQuery('DataSet');
  query.filter('repocode', repocode);
  ds.runQuery(query, cb);
}

exports.getTables = (dataset, cb) => {
  var bqds;
  if (dataset.bqowner) {
...
exports.getTable = (datasetName, tableName, cb) => {
  var bqds = bq.dataset(datasetName);
  var table = bqds.table(tableName);
  table.get(cb);
}

exports.queryTable = (datasetId, tableName, whereClause, cb) => {
...
exports.postTableData = (datasetId, tableName, data, cb) => {
  exports.getDataSet(datasetId, (err, dataset) => {
...
// table.metadata.schema.fields
        ...
        for(var i=0; i < lines.length; i++) {
          var entries = CSVtoArray(lines[i]);
          var row = {};
...
exports.createTable = (datasetName, tableName, tableSchema, cb) => {
  var bqds = bq.dataset(datasetName);
  bqds.createTable(tableName, { schema: tableSchema }, cb);
};

exports.createDataSet = (dataset, cb) => {
